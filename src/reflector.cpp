...
std::pair<bool, Vector> MirrorReflector::ReflectParticle(const Particle& pt,
    ...
    if(rnd(rnd_gen)>reflection_coefficient_){
        return std::make_pair(false, Vector(0.0,0.0,0.0)); 	//particle died
    }
    double vel_proj = pt.GetDirection().Dot(normal);
    Vector new_vel = pt.GetDirection().Times(2.0) +
                                            normal.Times(2.0*vel_proj);
    return std::make_pair(true, new_vel);
}
